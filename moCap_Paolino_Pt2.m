...
% Video information
vidFile = 'footballVid.mp4'; % with extension

% Load video
vid = VideoReader(vidFile);    % reads in .avi file
% More video information
videoRate = vid.FrameRate; % Default is 30 fps
timeStart = .65; % start time (s)
timeStop = 2;% stop time (s)
frameStart = ceil(timeStart*videoRate); % starting frame
frameStop = floor(timeStop*videoRate); % stopping frame

% Thresholding Red LED
RrLTH = 200; % red lower threshold
RgUTH = 70; % green upper threshold
RbUTH = 70; % blue upper threshold

% Thresholding Green LED
GrUTH = 50; % red upper threshold
GgLTH = 180; % green lower threshold
GbUTH = 210; % blue upper threshold

% Thresholding Cyan LED
CrUTH = 25; % red upper threshold
CgUTH = 230; % green upper threshold
CbLTH = 220; % blue lower threshold
...
for k = frameStart:frameStop
    ...
    imgBin2 = imgBin;
    imgBin2(imgBinRed == 1 | imgBinGreen == 1 | imgBinCyan == 1) = 1;
    imgBin2(~(imgBinRed == 1 | imgBinGreen == 1 | imgBinCyan == 1)) = 0;
    ...
    xlabel('X Position (pxs)')
    ylabel('Y Position (pxs)')
...
for k = frameStart:frameStop
    ...
    subplot(2,1,2)
    plot([RcCent(i)/px2m,GcCent(i)/px2m],[(rMax-RrCent(i))/px2m,(rMax-GrCent(i))/px2m],'g',...
        [RcCent(i)/px2m,CcCent(i)/px2m],[(rMax-RrCent(i))/px2m,(rMax-CrCent(i))/px2m],'r')
    hold on

    plot(RcCent(i)/px2m,(rMax-RrCent(i))/px2m,'r.', GcCent(i)/px2m,(rMax-GrCent(i))/px2m,'g.',...
        CcCent(i)/px2m,(rMax-CrCent(i))/px2m,'c.',MarkerSize = 30)
    hold on
